...
export function registerRecordTools(server: McpServer): void {
  ...
  server.tool(
    "ironclad_create_record_attachment",
    "Upload/attach a file to a record. Provide a download URL for the file.",
    {
      recordId: z.string().describe("The record ID"),
      name: z.string().describe("File name for the attachment"),
      downloadUrl: z.string().url().describe("URL to download the file from"),
      fieldId: z.string().optional().describe("Optional field ID to attach the file to a specific attribute"),
    },
    async ({ recordId, name, downloadUrl, fieldId }) => {
      try {
        const body: Record<string, unknown> = { name, downloadUrl };
        if (fieldId) body.fieldId = fieldId;
        const data = await api.post(`/records/${recordId}/attachments`, body);
        return successResult(formatJson(data));
      } catch (e) {
        return errorResult((e as Error).message);
      }
  ...
  server.tool(
    "ironclad_delete_record_attachment",
    "Delete a file attachment from a record. This action cannot be undone.",
    {
      recordId: z.string().describe("The record ID"),
      attachmentId: z.string().describe("The attachment ID to delete"),
    },
    async ({ recordId, attachmentId }) => {
      try {
        await api.delete(`/records/${recordId}/attachments/${attachmentId}`);
        return successResult(`Attachment ${attachmentId} deleted successfully.`);
      } catch (e) {
        return errorResult((e as Error).message);
      }
    }
  ...
  // ── Get Smart Import Status ──
  server.tool(
    "ironclad_get_smart_import_status",
    "Check the status of a smart import job.",
    {
      importId: z.string().describe("The smart import job ID"),
    },
    async ({ importId }) => {
      try {
        const data = await api.get(`/records/smart-import/${importId}`);
        return successResult(formatJson(data));
      } catch (e) {
        return errorResult((e as Error).message);
      }
